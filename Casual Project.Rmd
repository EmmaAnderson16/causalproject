...
#--------------------------------------------------
run.tmle <- function(final_data2, SL.library){
+
+ #------------------------------------------
...
+ X<- subset(final_data2, select=c("smoking", "age", "income", "access_care", "race_category", "education_category", "benzene_numerical"))
+
...
X_full <- subset(final_data2, select=c("smoking", "age", "income", "access_care", "race_category", "education_category", "benzene_numerical", "diabetes"))
+
+ # Estimate E_0(Y|A,W) with Super Learner
+ SL.outcome <- SuperLearner(Y=final_data2$diabetes, X=X, SL.library=SL.library,
family="binomial")
+
+ # get the expected outcome, given the observed exposure and covariates
+ expY.givenAW <- predict(SL.outcome, newdata=X_full)$pred
...
+ SL.exposure<- SuperLearner(Y=X_full$benzene_numerical,
X=subset(final_data2, select= c("smoking", "age", "income", "access_care", "race_category", "education_category")),
...
+
+ # clever covariate
+ H.AW <- as.numeric(final_data2$benzene_numerical==1)/probA1.givenW - as.numeric(final_data2$benzene_numerical==0)/probA0.givenW
+
...
+
+ # IPW estimate
+ PsiHat.IPW <- mean(H.AW*final_data2$diabetes)
+
+ #------------------------------------------
+ # Targeting & TMLE
+ #------------------------------------------
...
+ logitUpdate<- glm( final_data2$diabetes ~ -1 +offset(qlogis(expY.givenAW)) +
...
# 4. apply
result <- run.tmle(final_data2=final_data2, SL.library=SL.library)
round(out$estimates*100, 1)
...
+
+ # Super Learner for the exposure mechanism P_0(A=1|W)
+ SL.exposure<- SuperLearner(Y=dt$benzene_binary,
 X=subset(dt, select= -c("benzene_binary","diabetes")),
 SL.library=SL.library, family="binomial")
+
+ # generate the predicted prob of being exposed, given covariates
+ probA1.givenW <- SL.exposure$SL.predict
...
+
+ # clever covariate
+ H.AW <- as.numeric(dt$benzene_binary==1)/probA1.givenW - as.numeric(dt$benzene_binary==0)/probA0.givenW
+
+ # also want to evaluate the clever covariate at A=1 and A=0 for all participants
+ H.1W <- 1/probA1.givenW
+ H.0W <- -1/probA0.givenW
...
```{r}
final <- subset(final_data2, select=c("diabetes", "smoking", "age", "income", "access_care", "race_category", "education_category", "benzene_binary"))
...
set.seed(1)
out <- run.tmle(dt=final_data, SL.library=SL.library)
# Simple Subs
